...
# Jenkins实现Vue项目自动化部署
### 引言
    主要带大家一起见识下上层的持续集成（Continuous Integration）和持续交付/部署（Continuous Delivery/Deployment）的发展，
以及对 Framework开发 CI/CD 是否有可参考和借鉴的地方,当时第一次接触jenkins时，给我带来了很大的震撼.配置完成后,基本运维部署方面就没再操心过.由于当前不是技术文
更多的是演示为主(然后这里有个遗憾是我们的gitea没法开启webhook,所以当前的这个演示会有个瑕疵. 
需要手动去点下按钮才能部署成功)
...
### 结束语(Framework开发过程中的自动化建设的难题)
    对比上层开发,上层开发的自动化构建在工具链成熟度、依赖管理、构建速度、测试自动化、CI/CD 流程、跨平台支持和开发工具生态等方面都相当成熟,相比之下,目前我们Framework的开发自动化建设还处于起步阶段,主要体现在以下几个方面:
- 自动化构建工具链不完善
- 构建工具的作用不清楚或者不清楚如何使用
  传统的脚本方便但是容易忘记,不容易传承,举个例子:比如我修改了一个需求,验证通过了,想要提交代码时，需要比对各个修改后的文件，然后进行提交.原始的方法是,将修改的文件跟客制化目录下的文件比较，客制化文件存在就逐个比对，不存在就拷贝,然后我了解到这个是有实现py脚本来做这个工作的.
